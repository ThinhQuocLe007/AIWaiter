...
// Kiosk (entrance check-in: pick a free table + party size → POST /seatings).
...
export default defineConfig({
  ...
  server: {
    ...
    host: true,
    // Fixed port 5174 (port convention: customer_ui 5173 · kiosk 5174 · panel 5175).
    port: 5174,
    strictPort: true,
    // Same-origin proxy to FastAPI so there is no CORS (matches customer_ui/panel setup).
    proxy: {
      '/api': {

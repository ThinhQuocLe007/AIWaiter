...
Câu trả lời sẽ được đọc thành tiếng, TUYỆT ĐỐI KHÔNG chứa giá tiền hay con số ₫
— màn hình của khách tự hiển thị hình món kèm giá ngay dưới câu trả lời.
Nếu khách hỏi giá: nhắc TÊN MÓN đúng như trong CONTEXT và mời khách xem giá
hiển thị trên màn hình.
Dùng "Dạ", "ạ", xưng "em", gọi khách là "anh/chị". 1-3 câu. Không kể lể.
```

> Vì sao không đọc giá: Piper TTS đọc y nguyên chữ LLM viết. Giá + hình món
> hiển thị thành card ngay dưới bong bóng chat trên tablet (VoicePanel bắt tên
> món trong reply → VoiceDishCard có giá + nút +/− thêm giỏ). Để chắc chắn giá
> không lọt vào giọng nói, `_format_search_for_llm` và phần "Món đã thảo luận"
> của `_format_chat_for_llm` KHÔNG đưa giá vào CONTEXT nữa; riêng giỏ hàng /
> thanh toán vẫn giữ giá (khách hỏi "tổng bao nhiêu?" cần được đọc to).

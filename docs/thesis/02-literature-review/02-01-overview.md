...
Free-navigation platforms use LiDAR and RGB-D cameras for SLAM-based mapping and autonomous navigation in restaurant environments [2.1.1]–[2.1.3]. The category includes the Bear Robotics Servi, the Pudu BellaBot, and the Keenon T-series. These robots build an occupancy grid of the restaurant floor, localize within it, and plan collision-free paths between kitchen and tables. The scale of deployment is substantial: BellaBot was reported operating in over 600 cities across more than 60 countries as of June 2023, and Pudu reported nearly 70,000 cumulative units shipped across its full product range as of August 2023 [2.1.5]. All three platforms share one operational model. A human waiter loads food onto the robot and selects a destination table on a touchscreen, and the robot navigates autonomously to deliver the items. On arrival it plays a greeting, which on BellaBot accompanies a cat-like animated face, waits for the customer to retrieve the food, and returns to the kitchen. Load, navigate, deliver, and return are all autonomous; the ordering and conversational components remain entirely human-operated.
...
No category covers more than two of the three components, and none covers conversation and delivery together.
...
The delivery robot in this thesis is an open ROS2 platform of the second kind reviewed in §2.1.1, a two-wheel differential-drive base whose navigation stack §2.2 develops in detail.

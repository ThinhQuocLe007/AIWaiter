...
Bounded odometry drift: The fused encoder-IMU odometry must bound accumulated position and heading drift over a full delivery loop and must be corrected at the table against an absolute reference, since raw wheel odometry alone accumulates error beyond the arrival tolerance stated below.
...
*Table 3.: Robot platform specification.*
...
*Figure 3.: Block diagram of the robot's electronics with the Jetson Orin Nano at the center and three data links.*
...
*Table 3.: Extended Kalman Filter design settings.*

| Parameter | Value | Reason |
| --- | --- | --- |
| v | , , | Only the twist of the wheel odometry is fused; its integrated pose is ignored, so that pose and twist are not both taken from one source. |
| imu0_config | only | Only the gyroscope yaw rate is fused; orientation is excluded for want of a magnetometer. |
...
*Table 3.: Covariance values, planar diagonal entries.*
...
*Figure 3.: Block diagram of Encoder-IMU odometry fusion using EKF.*
...
*Table 3.: RTAB-Map configuration for mapping.*
...
*Figure 3.: RTAB-Map SLAM data-flow architecture.*

![image17.jpeg](images/image17.jpeg)

*Figure 3.: Effect of loop closure. The map before and after a loop closure is accepted*
...
*Figure 3.: Localization and pose-correction flow.*
...
Table 3.7 lists the critics and their weights . Collision safety is not one of them. A trajectory that enters a lethal costmap cell is discarded before it is scored at all, so the weighted sum only ever chooses between trajectories that are already collision-free.
...
*Table 3.: Global planner and local controller configuration.*
...
*Figure 3.: Autonomous navigation architecture and motion planning framework.*

![image20.png](images/image20.png)

Figure 3.: Sensor field-of-view and blind zone analysis (side view).CHAPTER 4: PROPOSED METHOD (II): AI AND BACKEND SYSTEM

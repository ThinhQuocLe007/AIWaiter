...
are also the properties recorded for each option by the voice survey in Section 2.3 of
Chapter 2:
...
**Voice-activity detection: Silero VAD.** Based on the survey of voice activity detection in
Section 2.3.1 of Chapter 2, Silero VAD is the natural choice. It is a small neural network,
...
larger detectors compared there, pyannote and NeMo, are more accurate but need the graphics
...
**Speech recognition: PhoWhisper medium.** Based on the survey of Vietnamese speech
recognition in Section 2.3.2 of Chapter 2, PhoWhisper at the medium size is selected. It is the
largest piece of the pipeline at about 3.5 GB. That figure is the memory the process holds
while running, not the size of the model on disk: the weights are about 1.5 GB at half
precision, and the graphics context, the working buffers, and the five-way beam search account
for the rest. PhoWhisper is the Whisper speech model fine-tuned on Vietnamese, which sharpens
...
runtime's own format and then loaded like any other model. The other options tabulated there
fall away against the yardstick: the cloud services are accurate but break the offline
requirement, the plain multilingual Whisper of the same size is weaker on Vietnamese tones, and the largest
...
**Speech synthesis: Piper.** Based on the survey of Vietnamese speech synthesis in Section
2.3.3 of Chapter 2, Piper is selected. It is a small neural voice, about two hundred megabytes,
that runs on the processor and speaks a sentence in roughly half a second. It is the only
Vietnamese voice compared there that runs fully offline, which is what decides it against the
three requirements: the cloud voices are more natural, but each spoken reply would then wait on
the network, the one thing the pipeline is built to avoid. With Piper the reply is spoken
entirely on the robot.

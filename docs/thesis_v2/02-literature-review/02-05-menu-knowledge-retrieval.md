...
Large language models inevitably hallucinate when queried about proprietary data like a restaurant's menu. Retrieval-Augmented Generation (RAG) mitigates this by fetching relevant domain documents into the prompt before generation. The literature categorizes RAG into three generations, Naive, Advanced, and Modular, which progressively improve and decouple individual pipeline stages. However, while Modular RAG makes components swappable, it completely lacks active coordination; nothing in the architecture dictates whether a module's output is adequate or if retrieval has failed.

All three standard architectures share a fragile assumption: that the query and the documents share the same vocabulary and vector space. This fundamentally fails when a user's need is expressed in terms absent from the corpus, a flaw documented as query-document vocabulary mismatch. Because the query and document vectors occupy entirely disconnected regions of the space, simply upgrading an encoder or a chunking strategy cannot repair the gap.

The root of this vulnerability is architectural: standard RAG enforces a strictly one-way information flow (query $\rightarrow$ retrieve $\rightarrow$ generate). The LLM is positioned at the very end of the sequence. It passively receives whatever retrieval returns, has no part in deciding how the search was conducted, and lacks any mechanism to register that the results are unusable. Consequently, the architecture provides no position to detect a vocabulary mismatch, as doing so would require the LLM to evaluate the retrieval and actively reformulate the query before final generation.

...
Two absences appear in the surveyed voice pipeline once the setting moves from a standard evaluation environment to a dining room.

The first is acoustic and empirical. The literature supplies no definitive values for three quantities the pipeline cannot run without: the frame-level decision threshold of the detector, the silence interval that declares an utterance finished, and recognition accuracy on restaurant-domain vocabulary. Existing evaluations assume quiet conditions, read speech, or telephony, and none addresses the defining acoustic property of a dining room, which is intelligible conversation carrying from an adjacent table. Noise of that kind is speech by every measure a detector or a recognizer applies to it, so no threshold value separates it from the customer at the microphone on acoustic evidence alone. These three quantities must therefore be treated as empirical, measured in the deployment rather than read from a configuration table.

The second concerns how the pipeline is driven. The three components surveyed here are indifferent to how they are invoked: a detector reports a speech probability per frame, a recognizer transcribes the segment it is handed, and a synthesizer speaks the text it is given, and none of them knows who asked, from where, or whether the answer is still wanted. Everything that decides the shape of a spoken turn therefore sits above them, in a layer the acoustic literature does not evaluate: what arms the microphone, how long it stays armed, and what happens to a reply already being spoken when the customer talks over it. Single-device assistants leave that layer implicit, because the microphone, the speaker, the user, and the trigger occupy one location, so arming is a wake word and interruption is a local decision. Separating them changes what the layer has to do rather than what the models have to do, and the quantities it then governs, the time from the end of speech to the start of a reply and the cost of withdrawing a reply midway, are properties of the arrangement that no published component evaluation reports.
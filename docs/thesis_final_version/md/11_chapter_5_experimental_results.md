...
All experiments in this chapter use Qwen2.5 14B Instruct Q6_K, served by Ollama with keep_alive = -1. The same model instance serves the tool-calling workers, the rewriters, and the response generator. Results that depend only on the trained classifier, the deterministic validator, or the retrieval indices are unaffected by the choice of language model and are identified as such where they appear.
...
Table 5.2: Robot hardware components and specifications.
...
The Jetson's unified memory is shared by the CPU and GPU, and the operating system kills processes that exceed it. Before this chapter's work begins, the navigation and localization stack built in Chapter 3 already holds approximately 3.7 GB. The voice pipeline described in Section 4.4 takes a further ~3.7 GB, filling nearly all the remainder. The consequence, that the LLM cannot run on the robot, is validated in Section 4.3.1 with measured resident-memory figures; the navigation experiments in Section 5.3 test whether the Chapter 3 stack performs correctly within this budget, and the latency experiments in Section 5.4.6 confirm that the server-side LLM placement adds no unacceptable delay.
...
Table 5.3: Software stack specifications.
...
Router arms are evaluated on identical items, so comparisons are paired and use McNemar's exact test on the discordant pairs, reported as the counts (b, c) with the exact p-value.
...
Figure 5.3: Stockroom facility with polished ceramic tile flooring, three aisle-bounding storage crates, Dock station marked with ArUco 6, and Table 1 with ArUco 1
...
Table 5.5: Odometry return-to-start error.
...
Figure 5.5: Runtime ROS 2 transform tree (TF tree)
...
Figure 5.6: Occupancy grid with Dock and Table 1

Figure 5.7:  Localized paths overlaid on restaurant map
...
Table 5.8:** **Navigation and docking performance with visual alignment enabled.
...
Table 5.9: Navigation and docking performance without visual alignment (Nav2 only).
...
Figure 5.8: Real-time dynamic obstacle avoidance sequence in the service lane
...
| Nav2 navigation success & Table 1 docking precision | Navigation and docking test |  | service success; ArUco visual align guarantees pinpoint table docking for touchscreen interaction. |
...
**Discussion. **The empirical evaluations confirm that the proposed ROS 2 navigation architecture satisfies all core design requirements defined in Section 1.3 for autonomous indoor AI waiter service, table-side ordering, and customer interaction. While raw EKF odometry accumulates significant drift over a single service-demand loop (), RTAB-Map graph-based SLAM, incorporating 2D LiDAR scan matching, visual bag-of-words loop closure, and ArUco landmark constraints, continuously bounds pose estimation errors within  on the global map grid.
...
This section carries the thesis's primary contribution. It follows the agent's execution path: classify (section 5.4.1), validate (section 5.4.2), execute and verbalize (section 5.4.3), retrieve (section 5.4.4), then evaluates the composition end to end (section 5.4.5) and against its cost (section 5.4.6). Datasets, metric definitions and the statistical protocol are settled in section 5.2; the model configuration is given in section 5.1.
...
There is deliberately no arm pairing the classifier with and against conversation context. The deployed router takes a sentence embedding and nothing else, so such a pair would run identical code on identical inputs.

Table 5.13: Accuracy and latency of the router arms
...
The latency columns carry the objective. The MLP answers in 9.0 ms at the median against 217.1 ms for the deployment model prompted zero-shot, a factor of twenty-four, and 189.0 ms for the 3B model, a factor of twenty-one. It is also the most stable arm: its 95th percentile sits 1.0 ms above its median, where the previous hybrid router's sits 720 ms above its own. Matching a language model's accuracy at a twenty-fourth of its median latency is the result the architecture was built to produce, and Section 5.4.6 returns to the stability half of it.

**Objectives 1 and 2 are met**. On a pooled set of 225 held-out utterances (Section 5.2), the MLP classifier reaches 92.9% at a median latency of 9.0 ms, significantly above both deterministic baselines under McNemar (p = 2.5 × 10⁻⁵ against the centroid, p = 3.3 × 10⁻⁶ against the previous hybrid router) and statistically indistinguishable from the deployment model prompted zero-shot (93.8% at 217.1 ms, p = 0.80), which it matches at a twenty-fourth of the median latency. On a separate 149-item single-intent set, the classifier reaches 95.3 % (142/149) against the 90 % target. A 39-case holdout set partitioned before training gives 38/39.

Both accuracy figures exceed the 90 % target, and both come from a trained classifier at a latency at least an order of magnitude below the prompted deployment model. Objective 2 asks for that combination, not for the classifier to out-predict a language model. On that reading both objectives are met with margin.
...
**Objective 3 is met**: no off-menu item reached a cart tool in any validated run, against 32 with the validator bypassed, and the gate achieved that without refusing valid work. The evidence for the second half is the deterministic set of Section 5.4.2, where all 15 ambiguous names were flagged rather than resolved silently and none of the 16 misspellings was force-matched, together with the negative control inside the adversarial set. Both halves of the leakage claim rest on a single run of 41 scenarios, and Section 5.5.2 records this limitation.


### 5.4.3 Multi-Intent Execution and Verbalization
...
Table 5.16: Multi-intent execution and verbalization
...
When excluding these execution-only failures, verbalization is highly sound: 95.8% (115 of 120) of runs with at least one executed intent are fully verbalized to the user.
...
Overall, **the multi-intent property is successfully met**. The system reliably tells the customer the complete story on most turns, with only a single, isolated pairing defect preventing perfect verbalization.


### 5.4.4 Knowledge Retrieval
Objective 4 requires the system to retrieve relevant dishes from the 234-entry menu in response to Vietnamese sensory descriptions rather than dish names, measured by recall and mean reciprocal rank at rank five. The pipeline is described in Section 4.6: a lexical index, a dense vector index over the same corpus, a fusion of the two rankings, and a gate that inspects the result before any of it reaches the language model. The dataset is 50 queries with graded relevance judgements across three difficulty levels, 18 easy, 19 medium and 13 hard. No language model is involved in the figures below, so they are exact.
...
Fusion does not improve on the lexical index. Both fused rankings match BM25 on hit rate and stay below it on the other three metrics, and no weighting recovers the gap. The sweep has a particular shape: equal weighting is the worst setting tested and is worse than BM25 alone, since the dense lane demotes exact matches often enough to cost hit rate outright; from roughly 1.5 : 1 upward the metrics are flat; and the best setting remains 1 : 0, which removes the dense lane. The deployment runs at 3 : 1, inside that plateau.

Swapping the fusion method changes little. Linear score fusion at equal weights lands within 0.01 of reciprocal rank fusion at 3 : 1 on every metric and matches it exactly on hit rate, which locates the effect in the lane weighting rather than in the arithmetic combining the ranks. Fusion does not need tuning; on this corpus the second ranking carries no information the first does not already have.

The reason is a property of the corpus rather than of either retriever, and it is the one Section 2.5 anticipated. Customers name dishes using the words printed on the menu, so lexical overlap between query and document is high and the vocabulary gap dense retrieval exists to bridge is narrow. A dense index earns its place where users and documents use different words for the same thing, which is what a menu corpus does not do.

**The Effect of Query Rewriting**

Table 5.17 measures the retriever on the customer's words as spoken. In deployment the search agent rewrites them into category terms first, so that configuration is measured separately. The two runs below are a matched pair over the same 50 queries and the same relevance judgements as Table 5.17, whose raw column they reproduce exactly.
...
Rewriting trades exact matching for coverage, which is the trade the design intends. The lexical lane loses on every metric, as expected: the rewrite replaces the words printed on the menu, which are precisely what that lane matches on. The dense lane gains where it is supposed to, its hit rate rising from 0.700 to 0.720. On the fused ranking the two effects do not cancel evenly. Recall at five rises from 0.551 to 0.577 and precision with it, while hit rate falls from 0.840 to 0.820, one query in fifty moving from a hit to a miss.

The aggregate therefore understates what rewriting does, because it averages two populations the rewrite treats oppositely. A query naming a dish is best served unrewritten, and a query describing a sensation cannot be served at all without the rewrite: "muốn ăn gì đó nóng hổi no bụng" moves from an R@5 of 0.25 to 0.75 once it becomes "cháo, lẩu, súp". Deploying the rewrite unconditionally is what costs the lexical lane its margin, and routing it by query type is the obvious improvement this evaluation points to.

Figure 5.9: Retrieval Quality by Query Difficulty

Easy and medium queries behave similarly, at hit rates of 0.889 and 0.895, and the easy set ranks a relevant dish first on nine occasions in ten. The hard set falls to a recall of 0.249 and a hit rate of 0.692, so four of thirteen return nothing relevant.

Eight queries in fifty return no relevant dish at all, and they divide into four groups that mean different things.

Table 5.19: The eight queries the fused ranking returns nothing relevant for, by cause.
...
Only the second group is a retrieval failure in the ordinary sense, and the retriever cannot repair it. Those queries describe an information need through terms absent from the menu text, and no index tuning closes a gap that lives in the corpus. It is the structural failure identified in section 2.5.1 of chapter 2, query and documents occupying disconnected regions of the vocabulary. What this analysis adds is that the remedy is a menu carrying the tags customers ask by, not a better index. The unwritten section is a content gap of the same kind, closed by writing the section. The first group belongs to the gate. Not one of the eight is a case where a retrievable dish existed and the fused ranking failed to surface it.

The lexical lane misses a different eighth query. It resolves "đồ nhắm lai rai với bia" and fails on "muốn ăn gì đó nóng hổi no bụng", which has four relevant dishes in the corpus and returns none of them, so on that ranking there is one true ordering failure. It is also the query the rewrite is built for: rewritten to category terms it scores a recall at five of 1.000, against 0.000 unrewritten. The single case where ranking is at fault is therefore the case the deployed pipeline already handles.

**Dual-Lane Gatekeeper**

The gate, defined in Section 4.6.3, admits a query only if the semantic lane or the lexical lane finds evidence of relevance, and otherwise returns nothing rather than passing weakly related documents to the language model.

It admitted 48 of the 50 queries and rejected 2, withholding nothing it should have admitted. The false-positive half of the requirement therefore holds: no query with a relevant dish on the menu was refused.

Five of the fifty queries ask for things the menu genuinely cannot answer, and the gate turns away two of them. The result on that half is therefore partial.

The three it admits are the cases Section 4.6.3 predicts. Pizza and sushi clear the semantic lane at cosine similarities of 0.29 and 0.30, against a threshold of 0.25 that answerable queries clear at 0.27, so no cutoff separates them. The motorbike query clears the lexical lane on "giá", a field every menu entry carries and a word no stop list can remove without discarding legitimate price questions. The response-layer grounding check of Section 4.5.6 stands behind the gate for these cases.

**Objective 4 is partially met**. The retriever finds a relevant dish in the top five for 42 of the 47 queries the menu can answer, 89.4 %, against a target of 90 %, and reaches a recall at five of 0.551 fused and 0.597 on the lexical lane alone, against a target of 0.70. The recall target was set without accounting for relevance sets larger than the cut-off: twenty of the fifty queries list more than five relevant dishes and one lists twenty-six, so a perfect ranking scores 0.802 and the 0.70 target asks for 87 % of a perfect ranking rather than for 70 % of the relevant dishes. Measured against what is achievable, the fused ranking delivers 71 % and the lexical lane 76 %.

Two limitations qualify that. The hard tier reaches 9 of 13 on hit rate but only 31 % of the recall its relevance sets allow, against 100 % on the easy tier and 76 % on the medium one, so the queries furthest from menu vocabulary remain the weakest by a wide margin.
...
Seven conversations spanning thirty turns were authored and all seven are reported. The experiment meets the N = 5 protocol of Section 5.2.1, and unlike the deterministic experiments its outcome varies between runs: 29 of 35 scenario runs pass, ranging from 5 of 7 in the weakest run to 7 of 7 in the strongest.
...
Four findings follow, each stated as a property of the architecture rather than of one transcript, because the turn at which a behavior appears is not stable across runs even when the outcome is.
...
Figure 5.10: Turn Latency by Intent Class
...
Figure 5.11: Share of turn latency by graph node
...
### 5.4.7 Voice Pipeline Latency
This section evaluates the independent latency of the three core audio pipeline components: Voice Activity Detection (VAD), Speech-to-Text (STT), and Text-to-Speech (TTS). All measurements were conducted directly on the robot's embedded Jetson Orin Nano edge computer using pre-recorded project datasets to accurately reflect processing costs based on utterance length. The hardware was tested in its deployed **25 W power mode** (running on a 12 V battery), with the unrestricted MAXN_SUPER mode provided for baseline comparison.

**Voice Activity Detection (Silero VAD):** Processing is highly efficient, requiring a median of just 1.3 ms per 32 ms audio frame on the CPU. However, the operational delay at this stage is dictated by a deliberate design parameter: a fixed **1.50 s trailing-silence window**. This static wait time is required to confirm the customer has finished speaking, preventing the system from prematurely clipping sentences.

**Speech Recognition (PhoWhisper - STT):** This is the most resource-intensive component of the pipeline, consuming approximately 3.5 GB of runtime GPU memory. The median real-time factor (RTF) is 0.525, meaning processing compute scales to roughly half the duration of the customer's audio. Unrestricting the board's power limits only yields a 14% speed improvement. This indicates that performance is primarily bottlenecked by the model's footprint rather than power constraints; future optimizations would require evaluating smaller checkpoints rather than scaling hardware power.

**Speech Synthesis (Piper - TTS):** Synthesizing a complete response takes a median of 0.986 s. However, because the system utilizes chunked playback (splitting sentences after clauses or filler particles like *“Dạ”*), the initial audio segment is generated and played in just **0.256 s**. Consequently, the total synthesis time does not directly dictate the initial perceived latency experienced by the user.

Table 5.21 summarizes the static latency metrics for each isolated component, comparing the deployed 25 W mode against the unrestricted MAXN_SUPER mode. In this table, each figure represents the median of the run, with the 95th percentile provided in parentheses. The overall end-to-end user experience from the end of the customer's speech to the robot's initial audio output is analyzed in Section 5.4.8.

Table 5.21: Voice pipeline component latency on the Jetson Orin Nano


| Component | Device | n | 25 W (deployed) | MAXN_SUPER |
| --- | --- | --- | --- | --- |
| VAD, per 32 ms frame | CPU | 2 000 | 1.3 ms (1.4) | 1.0 ms (1.1) |
| STT, per utterance (median audio 2.92 s) | GPU, float16 | 180 | 1.476 s (1.806) | 1.275 s (1.560) |
| STT, real-time factor | GPU, float16 | 180 | 0.525 (0.689) | 0.452 (0.593) |
| TTS, first spoken segment | CPU | 87 | 0.256 s (1.090) | 0.194 s (0.979) |
| TTS, complete reply (median 160 chars) | CPU | 87 | 0.986 s (3.337) | 0.749 s (2.527) |
| VAD trailing-silence window | n/a | fixed | 1.50 s | 1.50 s |


### 5.4.8 Full System Latency
This section aggregates the isolated component metrics from Sections 5.4.6 and 5.4.7 to evaluate the overall latency budget established in Chapter 4: the total duration from the customer finishing an utterance to the first synthesized audio reply.

Because the agent was evaluated on the server and the voice components on the robot's edge hardware, this composition is an **additive budget** of separately measured stages rather than an observed end-to-end distribution. Consequently, the 95th percentile (p95) values represent a worst-case upper bound rather than a true statistical percentile of a single turn. Two elements are excluded from the table below:

**Classifier and Validator Nodes:** These are already encompassed within the Agent's 1.61 s baseline.

**Network Latency:** Local access point isolation prevented direct measurement between the robot and server. However, round-trip times to the router are in the single-digit milliseconds, three orders of magnitude below the measured stages, and are recorded as an outstanding task in Section 5.5.2.

Table 5.22: Full-system latency budget.


| Stage | p50 (s) | p95 (s) | Source |
| --- | --- | --- | --- |
| STT, PhoWhisper medium | 1.476 | 1.806 | Section 5.4.7, measured |
| Agent, including classifier and validator | 1.610 | 4.130 | Section 5.4.6, measured |
| TTS, first spoken segment | 0.256 | 1.090 | Section 5.4.7, measured |
| Transcript to first audio | 3.34 | 7.03 | Sum of the above |
| VAD trailing-silence window | 1.500 | 1.500 | Design constant |
| End of speech to first audio | 4.84 | 8.53 | Including the silence window |
| Robot to server network | 0.018 | 0.045 | WebSocket over Netbird / Wifi LAN (measured) |

Analyzing this aggregated budget reveals three critical system characteristics:

**Processing Parity Between Edge and Server:** While Section 5.4.6 indicated that the language model dominates the server-side computation (91% of node time), deploying the system shifts the balance. Speech recognition on the edge hardware (44.2%) consumes nearly as much of the median turn as the server-side agent (48.2%). A faster edge recognizer is now equally as critical to the user experience as a faster language model.

**Marginal Budget Compliance:** The evaluation of the five-second response budget depends heavily on the defined starting point. Measured from the arrival of the transcript, the median turn is 3.34 s (well within the target). However, measured from the end of the customer's speech, which includes the mandatory 1.50 s VAD trailing-silence window, the system reaches **4.84 s**. This barely satisfies the five-second target with a tight 0.16 s margin, leaving the system highly vulnerable to regressions or longer text segments (e.g., waiting for a complete reply pushes the median to 5.57 s).

**Worst-Case Degradation:** The worst-case bound (p95) fails the budget entirely, reaching 8.53 s from the end of speech. The server agent is the primary contributor here, accounting for 58.8% (4.13 s) of this delay. Furthermore, increasing the board's power profile to the unrestricted MAXN_SUPER mode only reduces the median end-to-end time to 4.58 s. This marginal quarter-second gain confirms that hardware power limits are not the primary bottleneck, and addressing the latency tail requires deeper architectural optimization.
...
Table 5.23: Objectives against measured results.
...
| 7. Map-based navigation | ≥ 90 % success | 10 of 10 navigation runs completed | Met |
| 8. ArUco precision docking | ≤ 10 cm lateral, ≤ 8° heading | 1.57 cm lateral, 0.30° heading | Met |
| Multi-intent verbalization completeness | Not set; measured as diagnostic | 92.0 % fully verbalised | Met expectations |

Five of the six software objectives are met outright. The MLP classifier at 95.3 % exceeds the 90 % target on utterances classifiable from their words, and matches a prompted language model on the pooled set while answering in a twenty-fourth of the time. Retrieval nearly meets its hit-rate target (89.4 % vs 90 %) but recall at rank five falls short (0.551 fused, 0.597 lexical, against a target of 0.70), leaving the hybrid design unvindicated on this corpus. Fusion costs a third of the lexical lane's recall on exactly the sensory queries the objective names. The end-to-end conversations complete on 82.9 % of runs, and what varies is the language model's judgement rather than the deterministic layers, which wrote nothing incorrect in any run. Multi-intent turns produce correctly verbalized replies for 92.0 % of executed intents, as measured by the corrected verbalization rate in Section 5.4.3. One objective could not be evaluated. The speech pipeline was integrated and demonstrated end to end on the robot, its recognition accuracy was not quantitatively benchmarked (latency is measured separately in Section 5.4.7)
...
Table 5.24: Failures by responsible component.

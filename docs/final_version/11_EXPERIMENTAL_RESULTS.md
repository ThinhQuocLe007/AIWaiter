...
![image48.jpeg](media/image48.jpeg)
...
The three it admits are the cases Section 4.6.3 predicts. Pizza and sushi clear the semantic lane at cosine similarities of 0.29 and 0.30, against a threshold of 0.25 that answerable queries clear at 0.27, so no cutoff separates them. The motorbike query clears the lexical lane on "giá", a field every menu entry carries and a word no stop list can remove without discarding legitimate price questions. The gate correctly rejects 2 of the 5 queries the menu genuinely cannot answer. The three admitted queries share a term with legitimate menu entries: 'giá' appears in every dish record, while 'pizza' and 'sushi' clear the semantic lane at cosine similarities of 0.29 and 0.30, above the 0.25 threshold. The response-layer grounding check of Section 4.5.6 stands behind the gate for these cases.

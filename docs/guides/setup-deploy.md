...
| **TTS** | Piper (mặc định, offline) + edge-tts (cloud) + sounddevice + soundfile | cần `libsndfile1`, `libportaudio2`; ép cloud bằng `TTS_BACKEND=cloud` (xem [run-voice-vi.md](run-voice-vi.md)) |
...
> **Chọn mic:** không đặt `MIC_DEVICE_INDEX` thì VAD **tự chọn thiết bị thu có chữ "USB" trong tên**,
> ưu tiên hơn `default` của PortAudio. Lý do: Jetson Orin phơi ra 20 endpoint `APE` XBAR-ADMAIF ảo — mở
> được nhưng thu ra im lặng — và `default` có thể rơi vào đó hoặc vào route pulse mà SSH không với tới.
> Khớp theo *tên* chứ không ghim số, vì index đổi khi cắm cổng USB khác hoặc sau reboot.
> Muốn ép thủ công thì `MIC_DEVICE_INDEX` vẫn thắng.
>
